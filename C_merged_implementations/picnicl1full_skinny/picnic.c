...
int get_param_set(picnic_params_t picnicParams, paramset_t* paramset)
{
    ...
    case Picnic_L1_UR:
        ...
        paramset->numSboxes = 16;
        paramset->numRounds = 24;
                  ...
                  picnic_privatekey_t* sk)
{
    ...
    /* Generate a private key - 16 bytes */
    ...
    /* Generate a random plaintext block - 16 bytes */
    ...
    skinny_encrypt((uint8_t*)pk->plaintext, (uint8_t*)pk->ciphertext, (uint8_t*)sk->data, nonce, &paramset);
...
size_t picnic_signature_size(picnic_params_t parameters)
{
    ...
    case TRANSFORM_FS:
        ...
        return paramset.numMPCRounds * (paramset.digestSizeBytes + paramset.stateSizeBytes + numBytes(2 * paramset.numSboxes * paramset.numRounds) +  2 * paramset.seedSizeBytes) + numBytes(2 * paramset.numMPCRounds) + paramset.saltSizeBytes;
    case TRANSFORM_UR:
        return paramset.numMPCRounds * (paramset.digestSizeBytes + paramset.stateSizeBytes + 2 * numBytes(2 * paramset.numSboxes * paramset.numRounds) +  3 * paramset.seedSizeBytes) + numBytes(2 * paramset.numMPCRounds) + paramset.saltSizeBytes;
    default:
        return PICNIC_MAX_SIGNATURE_SIZE;
    }
}
...
int picnic_validate_keypair(const picnic_privatekey_t* privatekey, const picnic_publickey_t* publickey)
{
    ...
    uint8_t nonce[] = "000102030405060708090A0B";
    skinny_encrypt((uint8_t*)publickey->plaintext, (uint8_t*)ciphertext, (uint8_t*)privatekey->data, nonce, &paramset);
