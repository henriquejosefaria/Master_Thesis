...
int main(int argc, char** argv)
{
    ...
    switch(ALGORITHM){
        ...
        case 0: // average L1 FS VERSION
            printf("Runing 100 times Picnic example with parameter set: Picnic_L1_FS");
            for(int i=0;i<100;i++) picnicExample(1);
            printf("\nAverage signature size: %d\n",AVG_SIG_SIZE/100);
            break;

        case 1: // average L1 UR VERSION
            printf("Runing 100 times Picnic example with parameter set: Picnic_L1_UR");
            for(int i=0;i<100;i++) picnicExample(2);
            printf("\nAverage signature size: %d\n",AVG_SIG_SIZE/100);
            break;

        case 2: // RUN L1 FS VERSION
            picnicExample(1);
            printf("\nSignature size: %d\n",AVG_SIG_SIZE);
            break;

        case 3: // RUN L1 UR VERSION
            picnicExample(2);
            printf("\nSignature size: %d\n",AVG_SIG_SIZE);
            break;

        case 4: // RUN L1 FS AND L1 UR VERSION
            for (picnic_params_t params = 1; params < 3; params++) {
                picnicExample(params);
            }
            break;

        default: // RUN ALL VERSIONS
