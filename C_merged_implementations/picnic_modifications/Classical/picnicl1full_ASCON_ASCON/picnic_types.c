...
unsigned char** allocate_wots_signatures(paramset_t* params){
    unsigned char** signatures = malloc(sizeof(unsigned char*) * params->numMPCRounds);
    for(uint32_t i = 0; i < params->numMPCRounds; i++){
        signatures[i] = malloc(sizeof(unsigned char) * HASH_SIZE);
    }
    return signatures;
}

unsigned char** allocate_wots_signatures_pk(paramset_t* params){
    unsigned char** pks = malloc(sizeof(unsigned char*) * params->numMPCRounds);
    for(uint32_t i = 0; i < params->numMPCRounds; i++){
        pks[i] = malloc(sizeof(unsigned char) * HASH_SIZE);
    }
    return pks;

}

unsigned char** allocateSks(paramset_t* params){
    unsigned char** sks = malloc(sizeof(unsigned char*) * params->numMPCRounds);
    for(uint32_t i = 0; i < params->numMPCRounds; i++){
        sks[i] = malloc(sizeof(unsigned char) * HASH_SIZE);
    }
    return sks;
}

void allocateAuxiliarViews(unsigned char* views[5][3], paramset_t* params){

    for (int i = 0; i < 5; i++) {
        for(int i2 = 0; i2 < 3; i2++){
            views[i][i2] = malloc(sizeof(unsigned char) * params->stateSizeBytes);
        }
    }
}
...
void allocateProof(proof_t* proof, paramset_t* params)
{
    ...
    proof->wotsSignature = malloc(HASH_SIZE);
