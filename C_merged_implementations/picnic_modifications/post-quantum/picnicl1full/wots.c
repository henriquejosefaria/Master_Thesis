...
void gen_ur_wots_sk(unsigned char *sk, uint8_t* as1, uint8_t* as2, uint8_t* as3, uint8_t* gs1, uint8_t* gs2, uint8_t* gs3, uint32_t* privateKey, paramset_t* params){
    ...
    shake256(sk, HASH_SIZE, buf, inlen);
}

void gen_fs_wots_sk(unsigned char *sk, uint8_t* as1, uint8_t* as2, uint8_t* as3, uint32_t* privateKey, paramset_t* params){
...
void gen_chain(unsigned char* out, unsigned char* in, uint32_t start, uint32_t steps, uint32_t round)
{
    ...
    for (i = start; i < (start+steps) && i < WOTS_W; i++) {
        ...
        shake256(out, HASH_SIZE, buf, size_value * 2 + HASH_SIZE);
    }
}

void gen_wots_pk_from_sig(unsigned char** wots_sig_pk, unsigned char** sig, uint32_t lengths[5], uint32_t round)
{
    /*gen_chain(wots_sig_pk[round],   sig[round],   lengths[0], WOTS_W - lengths[0], round);
    gen_chain(wots_sig_pk[round+1], sig[round+1], lengths[1], WOTS_W - lengths[1], round);
    gen_chain(wots_sig_pk[round+2], sig[round+2], lengths[2], WOTS_W - lengths[2], round);
    gen_chain(wots_sig_pk[round+3], sig[round+3], lengths[3], WOTS_W - lengths[3], round);
    gen_chain(wots_sig_pk[round+4], sig[round+4], lengths[4], WOTS_W - lengths[4], round);*/
    for (int i = 0; i < 5; i++)
    {
...
void get_lengths_verify(uint32_t lengths[5], unsigned char* views[5][3], size_t lenBytes){
    ...
    //printf("did for\n");
    shake256(secretValue, HASH_SIZE, buf, lenBytes * 15);
