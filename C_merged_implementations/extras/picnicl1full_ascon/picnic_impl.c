...
static inline void permutationAND_verify(state* t[2], state* states[2], randomTape_t* rand, view_t views[2]){
  ...
  */
  ...
  t[0]->x0 = (t[0]->x0 & s1.x1) ^ (t1.x0 & states[0]->x1) ^ (t[0]->x0 & states[0]->x1) ^ r[0] ^ r[1];
  t[0]->x1 = (t[0]->x1 & s1.x2) ^ (t1.x1 & states[0]->x2) ^ (t[0]->x1 & states[0]->x2) ^ r[0] ^ r[1];
  t[0]->x2 = (t[0]->x2 & s1.x3) ^ (t1.x2 & states[0]->x3) ^ (t[0]->x2 & states[0]->x3) ^ r[0] ^ r[1];
  t[0]->x3 = (t[0]->x3 & s1.x4) ^ (t1.x3 & states[0]->x4) ^ (t[0]->x3 & states[0]->x4) ^ r[0] ^ r[1];
  t[0]->x4 = (t[0]->x4 & s1.x0) ^ (t1.x4 & states[0]->x0) ^ (t[0]->x4 & states[0]->x0) ^ r[0] ^ r[1];
  setASCONBit(views[0].communicatedBits, rand->pos, t[0]->x0);

  t[1]->x0 = getByte((uint32_t *)views[0].communicatedBits, rand->pos);
...
static inline void ROUND_verify(u8 C, state* states[2], randomTape_t* tapes, view_t* views[2]) {
    ...
    for(int i = 0; i < 2 ; i++){
        ...
        t_aux.x0 = ~states[i]->x0;
        t_aux.x1 = ~states[i]->x1;
        t_aux.x2 = ~states[i]->x2;
        t_aux.x3 = ~states[i]->x3;
        t_aux.x4 = ~states[i]->x4;
        t[i] = &t_aux;
...
/* 
  ...
  views => views
*/
void mpc_ASCON_verify(view_t* view1, view_t* view2, randomTape_t* tapes, paramset_t* params){
  ...
  // set hash output
  outlen = params->stateSizeBytes;
  uint32_t *out[2] = { malloc(sizeof(uint64_t)*2), malloc(sizeof(uint64_t)*2) };

  while (outlen > RATE) {
    for(int i=0;i<2;i++){
      memcpy(out[i], &states[i]->x0, sizeof(uint64_t));
      out[i] += sizeof(uint64_t);
    }
    P12_verify(states, tapes, views);
    outlen -= RATE; //outlen -=RATE;
  }
  for(int i=0;i<2;i++){
    memcpy(&out[i][RATE], &states[i]->x0, sizeof(states[i]->x0));
    memcpy(views[i]->outputShare,out[i],params->stateSizeBytes);
  }
}
                 ...
                 const uint32_t* plaintext, randomTape_t* tape, paramset_t* params)
{
    ...
    mpc_ASCON_verify( view1, view2, tape,params);
             ...
             view_t views[3])
{
    /* AND - LowMC
        ...
        INFO:
        só se usam 2 dos bits por iteração (a,b), (a,c) ou (b,c)
        player 0: a[0], b[0], c[0]
        player 1: a[1], b[1], c[1]
        player 2: a[2], b[2], c[2]
        ...
        out[0] = (a[0] & b[1]) ^ (a[1] & b[0]) ^ (a[0] & b[0]) ^ r[0] ^ r[1];

        out[1] = (a[1] & b[2]) ^ (a[2] & b[1]) ^ (a[1] & b[1]) ^ r[1] ^ r[2];

        out[2] = (a[2] & b[0]) ^ (a[0] & b[2]) ^ (a[2] & b[2]) ^ r[2] ^ r[0];
...
static inline void permutationAND(state* t[3], state* states[3], randomTape_t* rand, view_t views[3]){
  ...
  */
  ...
  for (uint8_t i = 0; i < 3; i++) {
      state t1 = *t[(i + 1) % 3];
      state s1 = *states[(i + 1) % 3];

      t[i]->x0 = (t[i]->x0 & s1.x1) ^ (t1.x0 & states[i]->x1) ^ (t[i]->x0 & states[i]->x1) ^ r[i] ^ r[(i + 1) % 3];
      t[i]->x1 = (t[i]->x1 & s1.x2) ^ (t1.x1 & states[i]->x2) ^ (t[i]->x1 & states[i]->x2) ^ r[i] ^ r[(i + 1) % 3];
      t[i]->x2 = (t[i]->x2 & s1.x3) ^ (t1.x2 & states[i]->x3) ^ (t[i]->x2 & states[i]->x3) ^ r[i] ^ r[(i + 1) % 3];
      t[i]->x3 = (t[i]->x3 & s1.x4) ^ (t1.x3 & states[i]->x4) ^ (t[i]->x3 & states[i]->x4) ^ r[i] ^ r[(i + 1) % 3];
      t[i]->x4 = (t[i]->x4 & s1.x0) ^ (t1.x4 & states[i]->x0) ^ (t[i]->x4 & states[i]->x0) ^ r[i] ^ r[(i + 1) % 3];
...
/* 
  ...
  views => views
*/
void mpc_ASCON(randomTape_t* tapes, view_t views[3], paramset_t* params){
  ...
  // set hash output
  outlen = params->stateSizeBytes;
  uint32_t *out[3] = { malloc(sizeof(uint64_t)*2), malloc(sizeof(uint64_t)*2), malloc(sizeof(uint64_t)*2) };
                 ...
                 size_t messageByteLength, signature_t* sig, paramset_t* params)
{
    ...
    for (uint32_t k = 0; k < params->numMPCRounds; k++) {
        ...
        //fprintf(stdout,"Size of the outputs in bytes: (%lu,%lu,%lu).\n Expected bytes size: %u",sizeof(views[k][0].outputShare),sizeof(views[k][1].outputShare),sizeof(views[k][2].outputShare),params->stateSizeBytes);
        //fflush(stdout);
        /*

        VERIFICAÇÕES PARA O PICNIC COM LowMC
