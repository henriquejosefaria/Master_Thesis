...
int main()
{
    ...
    unsigned char *signatures[collisions];
    unsigned char *sm;

    for(int aux=0;aux < collisions;aux++){
        sm = malloc(SPX_BYTES + SPX_MLEN);
        signatures[aux] = sm;
    }
    ...
    //MEASURE("", 1,attack_function(sm, &smlen, m, SPX_MLEN, sk));

    MEASURE("Starting attack..    ", 1, attack_function(signatures, (unsigned long *)&smlen, m, SPX_MLEN, sk));

    MEASURE("Verifying..          ", 1, crypto_sign_open2(mout, &mlen, signatures, smlen, pk));
