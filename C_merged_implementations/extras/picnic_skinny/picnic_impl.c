...
void mpc_SKINNY(randomTape_t* tapes, view_t views[3], const uint32_t* plaintext, uint32_t* slab, paramset_t* params)
{
    for (int i = 0; i < 3; i++) {
        skinny_hash(views[i].outputShare, views[i].inputShare, sizeof(views[i].inputShare), tapes, views);
    }
}
