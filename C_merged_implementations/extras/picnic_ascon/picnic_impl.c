...
void Commit(const uint8_t* seed, const view_t view, uint8_t* hash, paramset_t* params)
{
    HashInstance ctx;
...
void mpc_AND(uint8_t in1[3], uint8_t in2[3], uint8_t out[3], randomTape_t* rand, view_t views[3])
{
...
void mpc_ASCON(randomTape_t* tapes, view_t views[3], const uint32_t* plaintext, uint32_t* slab, paramset_t* params)
{
    // Não preciso de gerar estados nem fazer message append
    for(int i=0;i<3; i++){
        crypto_hash(views[i].outputShare, views[i].inputShare, sizeof(views[i].inputShare) , tapes, views);
    }
}



void mpc_LowMC(randomTape_t* tapes, view_t views[3], const uint32_t* plaintext, uint32_t* slab, paramset_t* params)
{
