...
int get_param_set(picnic_params_t picnicParams, paramset_t* paramset)
{
    ...
    case Picnic_L1_UR:
        ...
        paramset->numMPCRounds = 50;
        paramset->numMPCParties = 3;
        paramset->numSboxes = 40;
        paramset->numRounds = 10; // 1 vez P12() e duas P6() -> rduzido para metade para ter o mesmo tam.
    ...
    paramset->andSizeBytes = numBytes(paramset->numSboxes * 2 * paramset->numRounds);
                  ...
                  picnic_privatekey_t* sk)
{
    ...
    ascon_encrypt(pk->plaintext,pk->ciphertext,nounce,sk->data);
    //LowMCEnc((uint32_t*)pk->plaintext, (uint32_t*)pk->ciphertext, (uint32_t*)sk->data, &paramset);
...
size_t picnic_signature_size(picnic_params_t parameters)
{
    ...
    case TRANSFORM_FS:
        ...
        return paramset.numMPCRounds * (paramset.digestSizeBytes + HASH_SIZE + paramset.stateSizeBytes + numBytes(3 * paramset.numSboxes * paramset.numRounds) +  2 * paramset.seedSizeBytes) + numBytes(2 * paramset.numMPCRounds) + paramset.saltSizeBytes;
    case TRANSFORM_UR:
        return paramset.numMPCRounds * (paramset.digestSizeBytes + HASH_SIZE + paramset.stateSizeBytes + 2 * numBytes(3 * paramset.numSboxes * paramset.numRounds) +  3 * paramset.seedSizeBytes) + numBytes(2 * paramset.numMPCRounds) + paramset.saltSizeBytes;
    default:
        return PICNIC_MAX_SIGNATURE_SIZE;
    }
}
...
int picnic_validate_keypair(const picnic_privatekey_t* privatekey, const picnic_publickey_t* publickey)
{
    ...
    ascon_encrypt(publickey->plaintext, ciphertext, nounce, privatekey->data);
...
void print_signature(const uint8_t* sigBytes, size_t sigBytesLen, picnic_params_t picnic_params )
{
    ...
    for (size_t i = 0; i < params.numMPCRounds; i++) {
        ...
        printHex(label, (uint8_t *)proofs[i].wotsSignature, HASH_SIZE);
